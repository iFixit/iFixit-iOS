...
# Need to manually fix up iFixit-Info.plist
   ...
   sed -i '.bak' -e 's/com.dozuki.dozuki/com.dozuki.zeal/g' iFixit-Info.plist
   sed -i '.bak' -e 's/Dozuki/Zeal Support/g' iFixit-Info.plist
   sed -i '.bak' -e 's/>dozuki</>zeal</g' iFixit-Info.plist
   sed -i '.bak' -e 's/\[Config currentConfig\].dozuki = YES;/\[Config currentConfig\].dozuki = NO;/' Classes/iFixitAppDelegate.m
   sed -i '.bak' -e 's/\[Config currentConfig\].site = ConfigDozuki;/\[Config currentConfig\].site = ConfigZeal;/' Classes/iFixitAppDelegate.m
