...
{
    NSString *filename = [SDImageCache cacheFilenameForKey:key];
...
- (void)storeImage:(UIImage *)image imageData:(NSData *)data forKey:(NSString *)key toDisk:(BOOL)toDisk
{
    ...
    if ([memCache count] > 20) {
        NSMutableArray *keysToRemove = [NSMutableArray array];
        NSArray *allKeys = [memCache allKeys];
        // Remove half
        for (int i = 0; i < [allKeys count] / 2; i++)
            [keysToRemove addObject:[allKeys objectAtIndex:i]];
        [memCache removeObjectsForKeys:keysToRemove];
    }
...
// Modified to never delete images stored from bookmarked guides.
- (void)cleanDisk {
    NSArray *bookmarkedImages = [[GuideBookmarks sharedBookmarks] cachedImages];
    
    NSDate *expirationDate = [NSDate dateWithTimeIntervalSinceNow:-cacheMaxCacheAge];
    NSDirectoryEnumerator *fileEnumerator = [[NSFileManager defaultManager] enumeratorAtPath:diskCachePath];
    for (NSString *fileName in fileEnumerator) {
        // Exclude bookmarked images.
        if ([bookmarkedImages containsObject:fileName])
            continue;
